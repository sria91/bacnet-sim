...
impl BinaryOutput {
    pub fn new(device_id: DeviceId, instance: u32, name: impl Into<String>) -> Self {
        Self {
            device_id,
            object_id: ObjectId { object_type: ObjectType::BinaryOutput, instance },
    ...
    fn effective_value(&self) -> bool {
        self.priority_array.iter().find_map(|s| *s)
...
impl BacnetObject for BinaryOutput {
    ...
    fn read_property(&self, property_id: PropertyIdentifier, _array_index: Option<u32>) -> Result<PropertyValue, BacnetError> {
        match property_id {
            ...
            PropertyIdentifier::ObjectType       => Ok(PropertyValue::Enumerated(ObjectType::BinaryOutput as u32)),
            PropertyIdentifier::Description      => Ok(PropertyValue::CharacterString(self.description.clone())),
            PropertyIdentifier::PresentValue     => Ok(PropertyValue::Enumerated(self.effective_value() as u32)),
    ...
    fn write_property(&mut self, property_id: PropertyIdentifier, _array_index: Option<u32>, value: PropertyValue, priority: Option<u8>) -> Result<(), BacnetError> {
        if property_id == PropertyIdentifier::PresentValue {
            let pri = (priority.unwrap_or(16) as usize).saturating_sub(1).min(15);
            match value {
                PropertyValue::Null => { self.priority_array[pri] = None; }
                PropertyValue::Enumerated(v) | PropertyValue::Unsigned(v) => {
                    self.priority_array[pri] = Some(v != 0);
                }
                PropertyValue::Boolean(v) => { self.priority_array[pri] = Some(v); }
    ...
    fn all_properties(&self) -> Vec<(PropertyIdentifier, PropertyValue)> {
        vec![
            ...
            (PropertyIdentifier::ObjectType, PropertyValue::Enumerated(ObjectType::BinaryOutput as u32)),
            (PropertyIdentifier::Description, PropertyValue::CharacterString(self.description.clone())),
            (PropertyIdentifier::PresentValue, PropertyValue::Enumerated(self.effective_value() as u32)),
