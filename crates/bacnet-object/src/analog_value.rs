...
impl AnalogValue {
    pub fn new(device_id: DeviceId, instance: u32, name: impl Into<String>, units: EngineeringUnits) -> Self {
        Self {
            device_id,
            object_id: ObjectId { object_type: ObjectType::AnalogValue, instance },
...
impl BacnetObject for AnalogValue {
    ...
    fn read_property(&self, property_id: PropertyIdentifier, _array_index: Option<u32>) -> Result<PropertyValue, BacnetError> {
        match property_id {
            ...
            PropertyIdentifier::ObjectType       => Ok(PropertyValue::Enumerated(ObjectType::AnalogValue as u32)),
    ...
    fn write_property(&mut self, property_id: PropertyIdentifier, _array_index: Option<u32>, value: PropertyValue, _priority: Option<u8>) -> Result<(), BacnetError> {
        match property_id {
            ...
            PropertyIdentifier::Description => {
                if let PropertyValue::CharacterString(s) = value { self.description = s; Ok(()) }
                else { Err(BacnetError::InvalidDataType) }
            }
    ...
    fn all_properties(&self) -> Vec<(PropertyIdentifier, PropertyValue)> {
        vec![
            ...
            (PropertyIdentifier::ObjectType, PropertyValue::Enumerated(ObjectType::AnalogValue as u32)),
