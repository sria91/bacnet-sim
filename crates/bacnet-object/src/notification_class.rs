...
impl NotificationClass {
    pub fn new(device_id: DeviceId, instance: u32, name: impl Into<String>) -> Self {
        Self {
            device_id,
            object_id: ObjectId { object_type: ObjectType::NotificationClass, instance },
...
impl BacnetObject for NotificationClass {
    ...
    fn read_property(&self, property_id: PropertyIdentifier, _array_index: Option<u32>) -> Result<PropertyValue, BacnetError> {
        match property_id {
            ...
            PropertyIdentifier::ObjectType          => Ok(PropertyValue::Enumerated(ObjectType::NotificationClass as u32)),
            PropertyIdentifier::Description         => Ok(PropertyValue::CharacterString(self.description.clone())),
            PropertyIdentifier::NotificationClass   => Ok(PropertyValue::Unsigned(self.notification_class)),
            _ => Err(BacnetError::UnknownProperty),
        }
    }

    fn write_property(&mut self, _property_id: PropertyIdentifier, _array_index: Option<u32>, _value: PropertyValue, _priority: Option<u8>) -> Result<(), BacnetError> {
        Err(BacnetError::WriteAccessDenied)
    }

    fn tick(&mut self, _now: std::time::SystemTime, _delta: std::time::Duration) {}
    fn changed_since(&self, _since: Instant) -> Vec<PropertyIdentifier> { vec![] }
    fn all_properties(&self) -> Vec<(PropertyIdentifier, PropertyValue)> {
        vec![
            ...
            (PropertyIdentifier::ObjectType, PropertyValue::Enumerated(ObjectType::NotificationClass as u32)),
            (PropertyIdentifier::Description, PropertyValue::CharacterString(self.description.clone())),
            (PropertyIdentifier::NotificationClass, PropertyValue::Unsigned(self.notification_class)),
        ]
    }
}
