...
impl BinaryValue {
    pub fn new(device_id: DeviceId, instance: u32, name: impl Into<String>) -> Self {
        Self {
            device_id,
            object_id: ObjectId { object_type: ObjectType::BinaryValue, instance },
...
impl BacnetObject for BinaryValue {
    ...
    fn read_property(&self, property_id: PropertyIdentifier, _array_index: Option<u32>) -> Result<PropertyValue, BacnetError> {
        match property_id {
            ...
            PropertyIdentifier::ObjectType       => Ok(PropertyValue::Enumerated(ObjectType::BinaryValue as u32)),
    ...
    fn write_property(&mut self, property_id: PropertyIdentifier, _array_index: Option<u32>, value: PropertyValue, _priority: Option<u8>) -> Result<(), BacnetError> {
        match property_id {
            PropertyIdentifier::PresentValue => {
                match value {
                    PropertyValue::Boolean(v) => { self.present_value = v; self.last_changed = Instant::now(); Ok(()) }
                    PropertyValue::Enumerated(v) | PropertyValue::Unsigned(v) => { self.present_value = v != 0; self.last_changed = Instant::now(); Ok(()) }
    ...
    fn all_properties(&self) -> Vec<(PropertyIdentifier, PropertyValue)> {
        vec![
            ...
            (PropertyIdentifier::ObjectType, PropertyValue::Enumerated(ObjectType::BinaryValue as u32)),
            (PropertyIdentifier::Description, PropertyValue::CharacterString(self.description.clone())),
            (PropertyIdentifier::PresentValue, PropertyValue::Boolean(self.present_value)),
